...
const samplePrices = {
  en: {
    currency: 'USD',
    lifetime_display: '$129.99',
    annual_display: '$19.99',
    monthly_display: '$3.99',
  },
  uk: {
    currency: 'USD',
    lifetime_display: '$149.99',
...
describe('catalogByCurrency', () => {
  test('dedupes storefronts sharing a currency and keeps the first', () => {
    ...
    assert.equal(catalog.USD.lifetime_display, '$129.99');
    assert.equal(catalog.EUR.lifetime_display, '149,99 €');
    assert.equal(catalog.JPY.lifetime_display, '¥22,000');
  });

  test('preserves catalog order for the first row per currency', () => {
    const ordered = {
      ja: { currency: 'JPY', lifetime_display: '¥22,000' },
      en: { currency: 'USD', lifetime_display: '$129.99' },
      uk: { currency: 'USD', lifetime_display: '$149.99' },
    };
...
describe('resolvePrices', () => {
  ...
  test('returns the override row and note when a known currency is chosen', () => {
    ...
    assert.equal(resolved.row.lifetime_display, '¥22,000');
